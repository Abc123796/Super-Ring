...
-- 旋转图片
local EpicImage = Instance.new("ImageLabel")
EpicImage.Size = UDim2.new(0, 40, 0, 40)
EpicImage.Position = UDim2.new(0, 10, 0, 10)
EpicImage.BackgroundTransparency = 1
EpicImage.Image = "rbxassetid://1762586445009" -- 使用提供的图片ID
EpicImage.Parent = MainFrame

-- 图片旋转动画
local rotationConnection
local function startImageRotation()
    if rotationConnection then
        rotationConnection:Disconnect()
    end
    
    rotationConnection = RunService.Heartbeat:Connect(function(deltaTime)
        EpicImage.Rotation = (EpicImage.Rotation + 100 * deltaTime) % 360
    end)
end

local function stopImageRotation()
    if rotationConnection then
        rotationConnection:Disconnect()
...
ToggleButton.MouseButton1Click:Connect(function()
    ...
    ToggleButton.Text = ringPartsEnabled and "已启动" or "已关闭"
